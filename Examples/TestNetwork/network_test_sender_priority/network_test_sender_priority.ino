...
void loop() {
  ...
  if(millis() - interval > 5000 || isSent){
    uint8_t retval_0 = nd->SendDatagramImmediate(B_ADDRESS, ToS::LOW_PR, low_pr);
    uint8_t retval_1 = nd->SendDatagramImmediate(B_ADDRESS, ToS::MEDIUM_PR, med_pr);
    uint8_t retval_2 = nd->SendDatagramImmediate(B_ADDRESS, ToS::HIGH_PR, hi_pr);
    interval = millis();
    isSent = 0;
